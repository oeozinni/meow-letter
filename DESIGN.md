...
- **팔레트**: 세이지 `#d8dcc2` · 민트 `#dcebe2` · 라일락 `#e2d3ea` 순환, 잉크 `#211e19`. 표지 전용 페리윙클 `#a2bbd9`(사용자 키비주얼에서 샘플링, photo.jpg 모서리와 동일해야 함). 전면 고정 종이 그레인 오버레이(feTurbulence, multiply).
- **표지(#cover)**: 키비주얼 재현 — Archivo 계단식 헤들라인(Woo Sung Min / Birthday / Party), photo.jpg + HTML 말풍선(원본에 박힌 꼬리를 덮는 위치 고정), 26.08.04 · Arranged by. Choi Eojin · Seoul, Korea는 Bookk Myungjo. 리빌은 봉투 열린 뒤 시작.
...
- **타이포**: 본문 Bookk Gothic Lt/Bd(눈누 jsdelivr woff2), 대형 디스플레이('우성민 고양이'·'따랑해')는 Bookk Myungjo Bd, 라벨 붓글씨 Nanum Brush Script, 영문 라벨 Grenze Gotisch, 표지 헤들라인 Archivo. 다른 페이스 추가 금지.
